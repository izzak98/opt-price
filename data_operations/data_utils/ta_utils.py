...
def calculate_cumulative_return(df, period) -> Series:
    ...
    if period == 1:
        period = 2
    return (1 + df['Close'].pct_change()).rolling(window=period).apply(np.prod, raw=True) - 1
...
def calculate_skewness(df, period) -> Series:
    ...
    if period == 1:
        period = 2
    return df['Close'].pct_change().rolling(window=period).apply(lambda x: skew(x), raw=True)
...
def calculate_kurtosis(df, period) -> Series:
    ...
    if period == 1:
        period = 2
    return df['Close'].pct_change().rolling(window=period).apply(lambda x: kurtosis(x), raw=True)
