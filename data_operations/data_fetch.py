...
from data_utils.ta_utils import generate_technical_features
...
def init_folders() -> None:
    ...
    if not os.path.exists('options_data'):
        os.makedirs('options_data')
...
def main() -> None:
    ...
    options_tickers = config['tickers']['options']
    for grouping in config['tickers']['assets'].keys():
        ...
        for ticker in tqdm(tickers, desc=f"Processing {grouping}"):
            ...
            if ticker in options_tickers:
                df[["Close"]].to_csv(os.path.join('options_data', f"{ticker}.csv"))
